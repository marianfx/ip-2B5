...
public class Skeletonizer {
	...
	private Size elemSize		= new Size(3, 3);				// the size of the object. Usually = 3x3
	
	private Double threshold		= 150.0;					// indicates the threshold used for pixel-detection (number of things to intersect so they can form a valid thing - eg dots for line)
	...
	private void configureImage(){
		...
		// transforms the initial image from RGB (three channeled) to a binary matrix of pixels, which will have black pixels as true and white pixels as false (binary_inv)
		Imgproc.threshold(imageMatrix, internalMatrix, threshold, maxValue, Imgproc.THRESH_BINARY_INV);
		
		//configure parameters
		imageSize 		= internalMatrix.size();
//		skeletonType	= CvType.CV_8UC1;
//		color			= new Scalar(0);//black
		skeleton 		= new Mat(imageSize, skeletonType, color);						// init the skeleton
		
		// construct the element (a cross, in a 3 x 3 matrix)
		element		 	= Imgproc.getStructuringElement(elemType, elemSize);
	}
